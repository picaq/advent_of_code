...
makeGraph = (arr, size, diag) => {
  graph = Array.from({length: size}, () => Array(size).fill(0));
  arr.forEach( pair => process(pair, diag) );
  return graph;
}

process = ([start, end], diag) => {
  let min, max;
  if (start[1] === end[1]) {
  ...
  if (diag) {
    ...
    changeX = () => { return start[0] < end[0] ? x++ : x-- };
    changeY = () => { return start[1] < end[1] ? y++ : y-- };

    const max = Math.abs(start[0] - end[0]);

    for ( let i = 0; i <= max ; i++ ) {
      graph[changeY()][changeX()]++    
    }
  }
}

hydro = (arr, size, diag = false) => makeGraph(arr, size, diag).flat().filter( x => x > 1 ).length;

// console.log(hydro(arr, 10));
console.log(hydro(arr2, 1000));


// console.log(hydro(arr, 10, true));
// console.log(makeGraph(arr, 10, true));
console.log(hydro(arr2, 1000, true));
